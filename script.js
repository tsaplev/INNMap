...
$(window).load(function () {
  ...
  $("#areas tr").hover(
    function () {
      $("#"+$(this).attr("id"), svgdom).myAddClass("highlight");
      $("#"+$(this).data("addid"), svgdom).myAddClass("highlight");
    }, 
    function () {
      $("#"+$(this).attr("id"), svgdom).myRemoveClass("highlight");
      $("#"+$(this).data("addid"), svgdom).myRemoveClass("highlight");
    }
  );
...
$getTotalPpl = function() {
  var summ = 0;
  $('tr.selected').each(function() {
    summ += parseInt($(this).find('td').last().html().replace(' ', ''));
  });
  $('#counter').html(summ.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
}
